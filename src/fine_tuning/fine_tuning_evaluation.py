...
output_file = DATA_DIR / "fine_tuning" / "fine_tuning_evaluation_results.csv"
...
for item in data:
    if "contexts" not in item or not item["contexts"]:
        raise ValueError(
            f"Item sin 'contexts' RAG real: '{item.get('question','?')[:60]}'\n"
...
print("3. Cargando Embeddings (BAAI/bge-m3) via transformers directo...")
