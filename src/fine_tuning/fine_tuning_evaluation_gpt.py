"""
Evaluación RAGAS del fine-tuning con GPT-4o-mini como juez.
Input:  data/fine_tuning/fine_tuning_inference_results.json
Output: data/fine_tuning/fine_tuning_evaluation_gpt_results.csv
"""
import json
import os
import sys
...
input_file  = DATA_DIR / "fine_tuning" / "fine_tuning_inference_results.json"
output_file = DATA_DIR / "fine_tuning" / "fine_tuning_evaluation_gpt_results.csv"

print("1. Cargando datos de inferencia fine-tuning...")
...
for item in data:
    if "contexts" not in item or not item["contexts"]:
        raise ValueError(
            ...
            "Regenera fine_tuning_inference_results.json incluyendo los contextos recuperados."
        )
...
print(f"4. Iniciando evaluación RAGAS fine-tuning GPT ({len(data)}q × 2 métricas)...")
result = evaluate(dataset=dataset, metrics=metricas, run_config=configuracion)

print("\n" + "=" * 40)
print("RESULTADOS FINALES FINE-TUNING (GPT-4o-mini)")
print("=" * 40)
print(result)
