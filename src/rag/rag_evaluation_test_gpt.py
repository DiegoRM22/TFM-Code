"""
Evaluación RAGAS del RAG test set con GPT-4o-mini como juez.
Input:  data/rag/rag_inference_test_results.json
Output: data/rag/rag_evaluation_test_gpt_results.csv
"""
import json
import os
import sys
...
output_file = DATA_DIR / "rag" / "rag_evaluation_test_gpt_results.csv"

print("1. Cargando datos RAG test set...")
...
print(f"4. Iniciando evaluación RAGAS RAG test GPT ({len(data)}q × 5 métricas)...")
result = evaluate(dataset=dataset, metrics=metricas, run_config=configuracion)

print("\n--- RESULTADOS RAG TEST SET (GPT-4o-mini) ---")
print(result)
