"""
Evaluación RAGAS del RAG avanzado S2b con GPT-4o-mini como juez.
Input:  data/rag/advanced/rag_advanced_inference_dataset.json
Output: data/rag/advanced/rag_advanced_evaluation_gpt_results.csv
"""
import json
import os
import sys
...
output_file = DATA_DIR / "rag" / "advanced" / "rag_advanced_evaluation_gpt_results.csv"

print("1. Cargando datos RAG avanzado S2b...")
...
print(f"4. Iniciando evaluación RAGAS RAG avanzado GPT ({len(data)}q x 5 métricas)...")
result = evaluate(dataset=dataset, metrics=metricas, run_config=configuracion)

print("\n--- RESULTADOS RAG AVANZADO S2b (GPT-4o-mini) ---")
print(result)
