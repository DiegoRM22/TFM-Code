"""
Evaluación RAGAS del pipeline híbrido con GPT-4o-mini como juez.
Input:  data/hybrid/hybrid_inference_results.json
Output: data/hybrid/hybrid_evaluation_gpt_results.csv
"""
import json
import os
import sys
...
input_file  = DATA_DIR / "hybrid" / "hybrid_inference_results.json"
output_file = DATA_DIR / "hybrid" / "hybrid_evaluation_gpt_results.csv"

print("1. Cargando resultados de inferencia híbrida...")
...
print(f"4. Iniciando evaluación RAGAS híbrido GPT ({len(data)}q × 5 métricas)...")
result = evaluate(dataset=dataset, metrics=metricas, run_config=configuracion)

print("\n" + "=" * 40)
print("RESULTADOS FINALES HÍBRIDO (GPT-4o-mini)")
print("=" * 40)
print(result)
