"""
Evaluación RAGAS del híbrido avanzado S4 (EnsembleRetriever + reranking + HyDE + LoRA).
"""
import json
import sys
import pandas as pd
...
output_file = DATA_DIR / "hybrid" / "advanced" / "hybrid_advanced_evaluation_results.csv"
...
print(f"4. Iniciando evaluacion RAGAS hibrido avanzado S4 ({len(data)}q x 5 metricas)...")
result = evaluate(dataset=dataset, metrics=metricas, run_config=configuracion_lenta)

print("\n" + "=" * 40)
print("RESULTADOS FINALES HIBRIDO AVANZADO S4")
print("=" * 40)
print(result)
