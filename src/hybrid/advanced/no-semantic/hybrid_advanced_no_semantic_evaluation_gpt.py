"""
Evaluación RAGAS del híbrido avanzado no-semántico con GPT-4o-mini como juez.
Input:  data/hybrid/advanced/no-semantic/hybrid_advanced_no_semantic_inference_results.json
Output: data/hybrid/advanced/no-semantic/hybrid_advanced_no_semantic_evaluation_gpt_results.csv
"""
import json
import os
import sys
...
input_file  = DATA_DIR / "hybrid" / "advanced" / "no-semantic" / "hybrid_advanced_no_semantic_inference_results.json"
output_file = DATA_DIR / "hybrid" / "advanced" / "no-semantic" / "hybrid_advanced_no_semantic_evaluation_gpt_results.csv"

print("1. Cargando resultados inferencia hibrida avanzada no-semantica...")
...
print(f"4. Iniciando evaluacion RAGAS hibrido avanzado no-semantico GPT ({len(data)}q x 5 metricas)...")
result = evaluate(dataset=dataset, metrics=metricas, run_config=configuracion)

print("\n--- RESULTADOS HIBRIDO AVANZADO NO-SEMANTICO (GPT-4o-mini) ---")
print(result)
