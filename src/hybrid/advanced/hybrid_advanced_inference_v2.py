"""
Híbrido avanzado (S4) v2: EnsembleRetriever (BM25+Chroma) + CrossEncoder reranking + HyDE
+ generador fine-tuneado (Llama-3-8B + LoRA). Test set 14q.
v2: fix max_length 2048->4096 — chunks semánticos excedían 2048 tokens.
"""
import json
import re
import sys
...
output_file = DATA_DIR / "hybrid" / "advanced" / "hybrid_advanced_inference_results_v2.json"
...
print("2. Cargando LLM auxiliar para HyDE (llama3:8b Ollama)...")
llm_hyde = OllamaLLM(model="llama3:8b", temperature=0.0)

print(f"3. Cargando generador fine-tuneado ({adapter_path})...")
