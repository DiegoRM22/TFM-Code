"""
Híbrido avanzado inference con medición de latencia por pregunta (H4).
Ejecuta sobre 5 preguntas. Output incluye latency_s.
Latencia incluye: HyDE expand + retrieval avanzado + generación fine-tuned.
"""
import time
import json
import re
...
output_file = DATA_DIR / "hybrid" / "hybrid_advanced_latency_results.json"
...
results = []
print(f"\n4. Inferencia híbrida avanzada con latencia ({N_SAMPLES} preguntas)...")

for i, entrada in enumerate(golden_dataset):
    pregunta = entrada["question"]
    print(f"[{i+1}/{len(golden_dataset)}] Procesando...", flush=True)

    # Latencia incluye HyDE + retrieval avanzado + tokenización + generación
